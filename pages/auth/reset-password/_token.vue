<template>
  ...
  >
    ...
    <div class="mt-8">
      <div v-if="errors" class="bg-red-50 border-l-8 border-red-900 mb-2">
      ...
      <ValidationObserver v-slot="{ handleSubmit }">
        <form method="post" @submit.prevent="handleSubmit(submit)">
          ...
          <ValidationProvider
            rules="required|min:7|password:7|confirmed:password_confirmation"
            v-slot="{ errors }"
          >
            <div class="form-control">
              <label class="label">
              ...
              <div class="relative">
                ...
                <span
                  ...
                  :class="[passwordShow ? 'bg-blue-400' : 'bg-blue-600']"
                  @click="passwordShow = !passwordShow"
                  >{{ passwordShow ? "HIDE" : "SHOW" }}</span
                >
              </div>
            </div>
            <label class="label" v-if="errors[0]">
          ...
          <ValidationProvider
            rules="required|min:7|password:7"
            vid="password_confirmation"
            v-slot="{ errors }"
          >
            <div class="form-control">
              <label class="label">
                <span class="label-text font-bold">Comfirm Password</span>
              </label>
              <div class="relative">
                <input
                ...
                <span
                  class="absolute top-3 right-2 text-2xs font-bold cursor-pointer p-1 text-white rounded-md"
                  :class="[confirmPasswordShow ? 'bg-blue-400' : 'bg-blue-600']"
                  @click="confirmPasswordShow = !confirmPasswordShow"
                  >{{ confirmPasswordShow ? "HIDE" : "SHOW" }}</span
                >
              </div>
            </div>
            <label class="label" v-if="errors[0]">
          ...
          <button class="mt-2 btn btn-primary w-full" type="submit">Reset</button>
        </form>
      </ValidationObserver>
    </div>
  </div>
  ...
  methods: {
    async submit() {
      try {
        ...
        await this.$axios.post("/api/reset-password", this.form).then((response) => {
